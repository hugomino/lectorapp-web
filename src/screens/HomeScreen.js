...
export default function HomeScreen() {
  ...
  const fetchLastFinished = async () => {
    ...
    if (error) {
      console.error('Error fetching last finished books', error);
      return;
    }
  ...
  const fetchStats = async () => {
    ...
    setStats({
      booksFinished,
      pagesRead: formatNumber(pagesRead),
      streak: racha
    });
  };
  ...
  const goToTerminated = () => navigation.navigate('EliminarLibroTerminado');
  ...
  const markFinished = async (book) => {
    ...
    if (progressError) {
      console.error('Error al actualizar progreso:', progressError.message);
      return;
    }
    ...
    if (logError) {
      console.error('Error al guardar en logs:', logError.message);
      return;
    }
